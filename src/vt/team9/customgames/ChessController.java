		...
		case 1:
		{
			...
			if (board.Pieces_[x][y].highlight_)
			{
				...
				if (check)
				{
					...
					Toast.makeText(main.getApplicationContext(), "You are still in Check",
				...
				if (piece.getName().contains("pawn") && oldPiece.team_ == Board.NoTeam && oldX != x
						&& piece.team_ == Board.Team1)
				{
					board.removePiece(x, y - 1);
				}
				else if (piece.getName().contains("pawn") && oldPiece.team_ == Board.NoTeam
						&& oldX != x && piece.team_ == Board.Team2)
				{
					board.removePiece(x, y + 1);
				}
				...
				if (checkmate)
				{
					Toast.makeText(main.getApplicationContext(), "Checkmate", Toast.LENGTH_SHORT)
							.show();
				}
				...
				checkPromotion();
				Toast.makeText(main.getApplicationContext(), "You can now submit your move.",
						Toast.LENGTH_SHORT).show();
				...
				out: for (int i = 0; i < board.width_; i++)
				{
					for (int j = 0; j < board.length_; j++)
					{
						...
						if (temp.team_ == -currentTeam)
						{
							...
							if (board.Pieces_[2][0].highlight_ || board.Pieces_[3][0].highlight_
				...
				out: for (int i = 0; i < board.width_; i++)
				{
					for (int j = 0; j < board.length_; j++)
					{
						...
						if (temp.team_ == -currentTeam)
						{
							board.clearAllHighlights();
							temp.getMoves(i, j, board);
							if (board.Pieces_[6][0].highlight_ || board.Pieces_[5][0].highlight_
									|| board.Pieces_[4][0].highlight_)
							{
								flag2 = false;
								break out;
				...
				out: for (int i = 0; i < board.width_; i++)
				{
					for (int j = 0; j < board.length_; j++)
					{
						...
						if (temp.team_ == -currentTeam)
						{
							board.clearAllHighlights();
							temp.getMoves(i, j, board);
							if (board.Pieces_[2][7].highlight_ || board.Pieces_[3][7].highlight_
				...
				out: for (int i = 0; i < board.width_; i++)
				{
					for (int j = 0; j < board.length_; j++)
					{
						...
						if (temp.team_ == -currentTeam)
						{
							board.clearAllHighlights();
							temp.getMoves(i, j, board);
							if (board.Pieces_[6][7].highlight_ || board.Pieces_[5][7].highlight_
									|| board.Pieces_[4][7].highlight_)
							{
								flag2 = false;
								break out;
				...
				if (temp.team_ == -currentTeam)
				{
					...
					for (int m = 0; m < board.width_; m++)
					{
						for (int n = 0; n < board.length_; n++)
						{
							...
							if (board.Pieces_[m][n].highlight_ && possibleMoves[m][n] == true)
							{
