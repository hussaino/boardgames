...
public class CheckerBoard extends Board {
	...
	public void initBoard() {
		...
		for (int i = 0; i < width_; i += 2)
			Pieces_[i][0] = new Checker(Team1, "");
		for (int i = 1; i < width_; i += 2)
			Pieces_[i][1] = new Checker(Team1, "");
		for (int i = 0; i < width_; i += 2)
			Pieces_[i][2] = new Checker(Team1, "");

		for (int i = 1; i < width_; i += 2)
			Pieces_[i][5] = new Checker(Team2, "");
		for (int i = 0; i < width_; i += 2)
			Pieces_[i][6] = new Checker(Team2, "");
		for (int i = 1; i < width_; i += 2)
			Pieces_[i][7] = new Checker(Team2, "");
