...
public class UtilsServer {
	...
	private static final String URL_SERVLET_EXTENSION_JOIN = URL_SERVLET_EXTENSION_GAMES
			+ "/join";
	...
	private static final String URL_GAMES = URL_BASE
			+ URL_SERVLET_EXTENSION_GAMES;
	private static final String URL_GAMES_JOIN = URL_BASE
			+ URL_SERVLET_EXTENSION_JOIN;
	private static final String URL_USERS = URL_BASE
			+ URL_SERVLET_EXTENSION_USERS;
	private static final String URL_TEAMS = URL_BASE
			+ URL_SERVLET_EXTENSION_TEAMS;
	...
	public static void createNewGame(Handler handler, Game game) {
		createNewGame(handler, game.isPrivate(), game.isRanked(),
				game.getDifficulty(), game.getNumTeams(),
				game.getNumPlayersPerTeam(), game.getTimeLimitPerMove(),
				game.getTurnStrategy(), game.getPlayers().get(0));
	}

	public static void createNewGame(Handler handler, boolean isPrivate,
			boolean isRanked, int difficulty, int numTeams,
		...
		new ControllerHttpRequestAndParse<Game>().fetchAndParseRequests(
				handler, parser, postNewGameRequest);
	}
	...
	public static void submitNewGameState(Handler handler, Game updatedGame) {
		try {
			...
			StringEntity boardEntity = new StringEntity(UtilsJSON
					.getJSON(board).toString());
			...
			new ControllerHttpRequestAndParse<String>().fetchAndParseRequests(
					handler, parser, putNewGameStateRequest);
		} catch (UnsupportedEncodingException e) {
			MyLogger.logExceptionSevere(UtilsServer.class.getName(),
					"submitMoveToServer", "", e);
	...
	private static void getUserWithUrl(Handler handler, String url) {
		...
		HttpGet getRequest = new HttpGet(url);
		new ControllerHttpRequestAndParse<User>().fetchAndParseRequests(
				handler, parser, getRequest);
	}
	...
	private static void getTeamUsingUrl(Handler handler, String url) {
		...
		HttpGet getRequest = new HttpGet(url);
		new ControllerHttpRequestAndParse<Team>().fetchAndParseRequests(
				handler, parser, getRequest);
	}
	...
	public static void getAllGamesForUser(Handler handler, User usr) {
		...
		new ControllerHttpRequestAndParse<Game>().fetchAndParseRequests(
				handler, parser, getAllGamesForUserRequest);
	}
}
